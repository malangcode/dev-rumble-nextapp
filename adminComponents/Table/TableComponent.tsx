...
const TableManagement: React.FC = () => {
  ...
  const areas = ['All Areas', 'Main Hall', 'Window Side', 'Counter Side', 'Private Section'];
      ...
      <div className="mx-auto">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-4 mb-8">
          {[
            ...
            { label: 'Cleaning', value: tables.filter(t => t.status === 'cleaning').length, color: 'bg-blue-400' },
            { label: 'Total Capacity', value: tables.reduce((sum, t) => sum + t.seats, 0), color: 'bg-purple-500' }
          ].map((stat, index) => (
            <div key={index} className="bg-white/80 backdrop-blur-sm rounded-xl shadow-sm border border-gray-200 p-4">
        ...
        {selectedView === 'floor' && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-6">Floor Plan Layout</h3>
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
              {filteredTables.map((table) => (
                <div
                  key={table.id}
                  onClick={() => handleTableClick(table)}
                  className={`p-4 rounded-lg border-2 cursor-pointer transition-all hover:shadow-md ${getStatusColor(table.status)}`}
                >
                  ...
                  <div className="text-sm space-y-1">
                    ...
                    {table.customer && (
                      <p className="font-medium truncate">{table.customer}</p>
                    )}
                    {table.timeOccupied && (
                      <p className="flex items-center text-xs">
                        <Clock className="h-3 w-3 mr-1" />
                        {table.timeOccupied} - {table.estimatedFinish}
                      </p>
                    )}
                    {table.orderValue > 0 && (
                      <p className="text-xs font-medium">Rs. {table.orderValue}</p>
                    )}
                  </div>
                </div>
              ))}
        ...
        {selectedView === 'list' && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-6">Table List</h3>
                ...
                <tbody>
                  {filteredTables.map((table) => (
                    <tr key={table.id} className="border-b border-gray-100 hover:bg-gray-50">
                      ...
                      <td className="py-3 px-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium capitalize ${getStatusColor(table.status)}`}>
                          {table.status}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                      ...
                      <td className="py-3 px-4">
                        <span className="text-sm text-gray-600">
                          {table.timeOccupied ? `${table.timeOccupied} - ${table.estimatedFinish}` : '-'}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <span className="font-medium">
                          {table.orderValue > 0 ? `Rs. ${table.orderValue}` : '-'}
                        </span>
                      </td>
                      <td className="py-3 px-4">
        ...
        {selectedView === 'reservations' && (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6 mb-8">
            <h3 className="text-xl font-semibold text-gray-900 mb-6">Reservations</h3>
            <div className="grid gap-4">
              {filteredReservations.map((reservation) => (
                <div key={reservation.id} className="border border-gray-200 rounded-lg p-4 hover:bg-gray-50">
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <h4 className="font-semibold text-lg text-gray-900">{reservation.customerName}</h4>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {[
              { time: '12:45', message: 'Table 3 became available', type: 'success' },
              { time: '12:42', message: 'New reservation for Table 7 at 19:30', type: 'info' },
              { time: '12:40', message: 'Table 5 cleaning completed', type: 'success' },
              { time: '12:38', message: 'Table 1 order completed - Rs. 850', type: 'info' },
              { time: '12:35', message: 'Table 8 estimated finish time updated', type: 'warning' }
            ].map((update, index) => (
